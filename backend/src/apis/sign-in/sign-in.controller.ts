...
export async function signInController (request: Request, response: Response): Promise<void> {
	try {
		...
		const isPasswordValid = await validatePassword( profile.hash, password,)
		...
		// deconstruct the id, about, imageUrl and name from the profile
		const { id, about, imageUrl, name} = profile

		// generate a new signature for the session
		const signature: string = uuid()

		// generate a new jwt for the session using the id, about, email, imageUrl, name and signature
